...
function getCashFlows( quote, request, section )
{
	$("#cashFlowsTable").remove();
	$("#cashHeader").remove();
	$("#" + section + "Section").append("<p id='loading'>Fetching data, please be patient!</p>");
	var requestURL = 'https://query.yahooapis.com/v1/public/yql?q=SELECT%20*%20FROM%20yahoo.finance.cashflow%20WHERE%20symbol%3D%27' + quote + '%27%20AND%20timeframe%3D%27annual%27&format=json&diagnostics=true&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=';
	...
	$.ajax(
    {
        ...
        async: false,
        success: function ( json ) {
        	var sArray = json['query']['results']['cashflow']['statement'];
        	console.log( sArray );
        	if( typeof sArray != "undefined" ) 
        	{
        		if( request == 0 )
       			...
       			$("#" + section + "Section" ).append("<p id='cashFlowsTable'>We're sorry, this quote does not appear to have any data.</p>");
       			$("#loading").remove();
       		}
        }
    });
